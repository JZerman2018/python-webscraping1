...
def create_csv_path(csv_path):
    if not os.path.exists(path):
        with open(path, 'w') as csvfile:  # open that path w = write/create
...
if "joincfe.com" in saved_domains:
    div_class = saved_domains['joincfe.com']
    print(div_class)
...
response = requests.get(my_url) # go to url and get it
...
if response.status_code != 200: # not equal, == equal
    ...
    if domain in saved_domains:
     ...
     #soup.find("body") # soup.find_all("a")
    # print(body_.text)
    words = body_.text.split() # remove stop words
